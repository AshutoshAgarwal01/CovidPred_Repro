...
logging.basicConfig(level=logging.DEBUG, filename="combined_training_logfile_batch_32.txt", filemode="w+", format="%(asctime)-15s %(levelname)-8s %(message)s")
    ...
    for i in range(0, total_iterations + 1):
        ...
        if i % epoch_size == 0: 
            ...
            if (epoch == 1 or epoch % 3 == 0 or epoch == total_epochs):
                ...
                saver.save(session, f'c:/temp/Model_{filename}_batch_32/trained_model') ### To save model with the name specified in this line
...
def createPlot(datasetname, title, epoch_array, acc_array, val_acc_array, val_loss_array):
    ...
    plt.savefig(f"c:/temp/Model_{datasetname}_batch_32/{filename}.png")
...
# Configuration for original and single augmented data.
# batch_size = 16
# final_iter = 529 # (For 24 Epochs, 23 iterations per epoch, batch size 16, repro dataset)

# Configuration for combined model when batch size = 32
batch_size = 32 # Use it only for combined dataset and final_iter = 23 * 288
final_iter = 23 * 299 # (For 24 Epochs, 23 iterations per epoch, batch size 32, original + augmented combined repro dataset)
# final_iter = 23 * 288 # (For 24 Epochs, 23 iterations per epoch, batch size 32, all augmented combined repro dataset)

# Configuration for combined model when batch size = 16
# batch_size = 16 # Use it only for combined dataset and final_iter = 23 * 576
# final_iter = 23 * 599 # (For 24 Epochs, 23 iterations per epoch, batch size 16, original + augmented combined repro dataset)
# final_iter = 23 * 576 # (For 24 Epochs, 23 iterations per epoch, batch size 16, all augmented combined repro dataset)
